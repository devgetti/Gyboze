...
windowManager.prototype.add = function(win, parent) {
	if(!win) return null;
	if(win.objectId) { this.del(win); }
	
	var objectId = Math.random().toString(36).slice(-8);;
	win.objectId = objectId;
	win.parentId = null;
	if(parent) {
		if(parent instanceof require('ui/baseWindow') || parent instanceof require('ui/baseTab')) {
			for(var id in this.objects) {
				if(this.objects[id] === parent) {
					win.parentId = id;
					break;
				}
			}
			if(!win.parentId) {
				win.parentId = this.add(parent);
			}
		}
	}
	this.objects[objectId] = win;
	return objectId;
};

windowManager.prototype.del = function(objectId) {
	if(this.object[objectId]) {
		delete this.object[objectId];
	}
};

windowManager.prototype.get = function(objectId) {
