...
exports.Cyboze = (function(global) {
  var K = function(){}, isAndroid = Ti.Platform.osname === "android", jsOAuth = require('lib/jsOAuth-1.3.6_for_Ti3.0.2');
  
  /**
   * Cyboze constructor function
   *
   *     var client = Cyboze({
   ...
   * @param [options.windowTitle="Cyboze Authorization"] {String} (optional) The title to display in the authentication window
   */
  var Cyboze = function(options) {
    var self;
    
    if (this instanceof Cyboze) {
      self = this;
    } else {
      self = new K();
    }
    ...
    if (!options) { options = {}; }
    self.windowTitle = options.windowTitle || "CybozeLive Authorization";
    ...
    self.authorizeUrl = "https://api.cybozulive.com/oauth/authorize";
    self.verifyUrl = "https://api.cybozulive.com/oauth/verify";
    self.xauthorizeUrl = "https://api.cybozulive.com/oauth/token";
    ...
    options.requestTokenUrl = options.requestTokenUrl || "https://api.cybozulive.com/oauth/initiate";
    self.oauthClient = jsOAuth.OAuth(options);

	return self;
  };
  
  K.prototype = Cyboze.prototype;
  ...
  function createAuthWindow() {
    ...
    webView.addEventListener('load', function(event) {
      // If we're not on the Cyboze authorize page
      if (event.url.indexOf(self.verifyUrl) === -1) {
        ...
        var pin = event.source.evalJS("document.getElementById('oauth_verifier').innerText");
        ...
        if (!pin) {
          ...
          oauth.accessTokenUrl = "https://api.cybozulive.com/oauth/token";
...
//            var returnedParams = oauth.parseTokenRequest(data.text);
  ...
  /*
   * Requests the user to authorize via Cyboze through a modal WebView.
   */
  Cyboze.prototype.authorize = function() {
    var self = this;
  ...
  /*
   * Make an authenticated Cyboze API request.
   * 
   * @param {String} path the Cyboze API path without leading forward slash. For example: `1/statuses/home_timeline.json`
   * @param {Object} params  the parameters to send along with the API call
   * @param {String} [httpVerb="GET"] the HTTP verb to use
   * @param {Function} callback
   */
  Cyboze.prototype.request = function(path, params, headers, httpVerb, callback) {
  ...
  Cyboze.prototype.logout = function(callback){
  	var self = this;
   ...
   */
  Cyboze.prototype.addEventListener = function(eventName, callback) {
   ...
   * Fire an event
   */
  Cyboze.prototype.fireEvent = function(eventName, data) {
